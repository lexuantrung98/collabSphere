...
namespace CommunicationService.Controllers
{
    ...
    {
        private readonly CommunicationService.Data.AppDbContext _context;
        ...
        public NotificationController(CommunicationService.Data.AppDbContext context, ILogger<NotificationController> logger)
        {
            _context = context;
            _logger = logger;
        }
        ...
        public async Task<IActionResult> SaveFcmToken([FromBody] SaveFcmTokenRequest req)
        {
            try
            {
                ...
                {
                    var newToken = new CommunicationService.Models.UserFcmToken
                    {
                        UserId = userId,
                        Token = req.Token,
                ...
                _logger.LogInformation("✅ Saved FCM token for user {UserId}", userId);

                return Ok(new { success = true, message = "Token saved" });
            }
            catch (Exception ex)
            {
